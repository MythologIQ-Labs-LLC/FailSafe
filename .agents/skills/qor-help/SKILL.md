...
**Standard feature**: `/qor-research` → `/qor-plan` → `/qor-audit` → `/qor-implement` → `/qor-substantiate` → `/qor-repo-release`

**Quick fix (L1)**: `/qor-plan` → `/qor-implement` → `/qor-substantiate`

**Diagnostics**: `/qor-status` → (routes to next action)

Full routing table: `.Codex/commands/references/qor-skill-routing.md`
