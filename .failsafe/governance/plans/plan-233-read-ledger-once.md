...
**iteration**: 5 (iter 1 VETOed — ledger #592, V1 evidence-format + V2 versionStatus drop; iter 2 VETOed — ledger #593, V3 half-honored option type + V4 unreachable test states; iter 3 VETOed — ledger #594/#595, B2 unreachable-fixture-state + B3 unpinned parse count, escalated to `/qor-remediate` per the third-consecutive-VETO rule; iter 4 VETOed — ledger #597, fourth consecutive, B3 resolved but B2 out of that iteration's authorized scope and left unresolved, execution stopped pending owner check-in per the operator's standing instruction for that contingency)
...
## Resolution of iteration-4 VETO finding (ledger #597)

**B2 (Phase 1's second test bullet claimed a no-options call against one of the six named `qor-consumer` fixtures reaches `unavailable`/absent; no fixture can, because all six ship a `docs/META_LEDGER.md`).** Iteration 4 was authorized to touch only five specific things and this bullet was not one of them, so it was correctly left unresolved and the cycle stopped rather than silently narrowing or patching out of scope — recorded at ledger #597 and PR #433.

**Owner decision (PR #433, comment 2026-08-23T16:56:25Z):** "proceed with iteration 5 by creating a genuine absent-ledger test condition (dedicated fixture or deterministic temp workspace) so the `unavailable` claim remains real and falsifiable. Do not narrow the contract merely to fit the existing fixtures." This resolves the B2 scope hold; the choice between the two named mechanisms is made below.

**Choice: deterministic temp workspace, not a seventh fixture.** Three independent pieces of this repo's own convention favor the temp-workspace mechanism over adding a fixture:

1. `src/test/roadmap/WorkspaceArtifactBuilder.test.ts`'s `makeWorkspace()` already covers the exact analogous case — "missing META_LEDGER.md" — with a bare `fs.mkdtempSync` workspace that creates a `docs/` directory but never writes a ledger file into it, not with a `qor-consumer` fixture.

   > `git show HEAD:FailSafe/extension/src/test/roadmap/WorkspaceArtifactBuilder.test.ts | grep -nE 'missing META_LEDGER.md' -> 24:  test("missing META_LEDGER.md → shieldPhase IDLE, derivedShieldPhases all pending", () => {`

2. `src/test/qorlogic/consumer/consumer-adapter.test.ts`'s own `#233 route-seam equivalence` suite already uses a direct `fs.mkdtempSync` temp workspace instead of `materialize()` and the six named fixtures, for the same reason — a state the fixture set does not carry.

   > `git show HEAD:FailSafe/extension/src/test/qorlogic/consumer/consumer-adapter.test.ts | grep -nE 'const ws = fs.mkdtempSync' -> 168:    const ws = fs.mkdtempSync(path.join(os.tmpdir(), 'consumer-233-'));`

3. `src/test/fixtures/qor-consumer/README.md` explicitly documents the set as "Six fixture workspaces" and enumerates all six by name. A seventh fixture would falsify that documented count and would require touching a file outside this plan's `Affected Files` list for no benefit the temp-workspace mechanism doesn't already provide.

   > `git show HEAD:FailSafe/extension/src/test/fixtures/qor-consumer/README.md | grep -nE 'Six fixture workspaces' -> 3:Six fixture workspaces for the Qor-logic consumer adapter tests: \`supported\`,`

Independently re-verified this cycle (not merely trusted from ledger #594/#597): `find src/test/fixtures/qor-consumer -iname META_LEDGER.md` returns exactly six hits, one per named fixture — none absent.

The Phase 1 test bullet below is rewritten accordingly: the six-fixture equivalence claim now covers only the four states the fixtures actually carry (`ok`/`malformed`/`stale`/`unsupported`), and a separate, explicitly-named temp-workspace case covers `unavailable`. FX892's MODIFIED descriptor (Feature Inventory Touches, below) is restated to match — this is the second half of what ledger #597 flagged, since the descriptor repeated the same unqualified six-fixture claim.
...
- `src/test/qorlogic/consumer/consumer-adapter.test.ts` — for each of the six `src/test/fixtures/qor-consumer/*` workspaces, `readMetaLedgerArtifact(root, opts)` returns an envelope whose `state`, `reason`, `provenance`, and `data` length equal those of the pre-change implementation, **with each state driven by the options that actually reach it**: `unsupported` via `{versionStatus: BELOW_FLOOR}` (mirroring the existing case at `consumer-adapter.test.ts:109`), `stale` via `{maxAgeMs: 1}` plus an `fs.utimesSync` mtime rewind (mirroring `:118-121`), and `ok`/`malformed` with no options. **iteration-5 B2 remediation (ledger #594/#597, owner decision on PR #433):** none of the six named fixtures ships an absent `docs/META_LEDGER.md` (re-verified this cycle: `find src/test/fixtures/qor-consumer -iname META_LEDGER.md` returns exactly six hits), so `unavailable` is no longer claimed against the fixture set. A seventh, separate case instead constructs a genuine absent condition directly — `fs.mkdtempSync` with no `docs/` directory ever created, the same direct-temp-workspace technique this file's own `#233 route-seam equivalence` suite already uses (`consumer-adapter.test.ts:168`) and `WorkspaceArtifactBuilder.test.ts`'s `makeWorkspace()` already uses for its analogous "missing META_LEDGER.md" case (`WorkspaceArtifactBuilder.test.ts:24`) — and asserts `readMetaLedgerArtifact(emptyRoot)` (no options) returns `state: 'unavailable'`, `data: null`, and a `reason` naming the source path, mirroring the existing `classifyMetaLedgerText` absent-state assertions (`consumer-adapter.test.ts:204-211`) but now exercised end-to-end through `readMetaLedgerRaw` and the redefined `readMetaLedgerArtifact`, not just the text-seam classifier alone. Confirms the redefinition is behavior-preserving across all five states, with `unavailable` now falsifiable rather than asserted against fixtures that cannot produce it — a bare no-options call against the six fixtures reaches only `ok`/`malformed` and would classify the `stale` and `unsupported-version` fixtures as `ok` while passing, which is why those two states are driven by explicit options above.
...
| FX892 | MODIFIED | `src/test/qorlogic/consumer/consumer-adapter.test.ts` | `readMetaLedgerArtifact` routed through `readMetaLedgerRaw` yields envelopes equal to the prior path across all six `qor-consumer` fixtures (`ok`/`malformed`/`stale`/`unsupported`), plus a directly-constructed absent-ledger temp workspace proving `unavailable` |
