...
- RUN: `/qor-repo-scaffold` silently
...
âš ï¸ **Privacy Reminder**: All `/qor-*` commands that create or modify files
will verify paths against gitignore before writing in public repos.

To modify privacy settings later, use `/qor-organize`.
```
...
| L3    | "DNA Seeded. CRITICAL: Security path detected. /qor-audit MANDATORY. Dataset LOCKED." |

**Note**: For non-trivial features in an existing workspace, use `/qor-research` (Strategist) before `/qor-plan` (Governor) to secure intent with evidence. Bootstrap is for **workspace genesis only**.
