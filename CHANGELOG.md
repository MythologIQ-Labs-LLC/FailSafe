...
- **B197 qor-logic version-floor surfacing** (`docs/plan-qor-logic-version-pinning.md`). Installer (`QorLogicPackageInstaller.ts`) had already pinned `qor-logic>=0.31.1` with `--upgrade` and implemented `verifyInstalledVersion()`; residual cycle wires the verifier into the hub artifact builder (resolved once per hub rebuild with graceful-degrade try/catch) so the Settings "Install / Refresh Skills" card now renders an explicit floor-violation warning when the installed version is below `MIN_QOR_LOGIC_VERSION`. 6 new SG-035 functional cases (FX511). 1-cycle architect-reviewer PASS with 3 minor amendments (construction site + async boundary + escape helper name) remediated in-cycle.
