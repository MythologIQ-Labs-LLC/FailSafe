...
- **B199 Phases 5-9 — full Command Center tab coverage + WS broadcast matrix + bus-renderer E2E (consolidated 2026-05-19)**. Phase 5 (Workspace tab structural — Skills/Mindmap sub-pills, FX521). Phase 6 (Governance tab structural — Audit/Risks/Compliance sub-pills, FX522). Phase 7 (Overview tab — default-active + tab-switch round-trip + no-error baseline, FX523). All 6 top-level Command Center tabs now have structural Playwright coverage. Phase 8 (WS broadcast matrix — 16 broadcast types covered: hub.refresh / event / verdict / transparency / agentRun / bicameral.{connected,disconnected} / brainstorm.reset / risk.{created,updated,deleted} / skills.install.{progress,complete} / voicePack.install.{complete,error} / voicePack.uninstalled — FX524, closes deep-audit HIGH "only 1 of ~12 covered"). Phase 9 (real disk META_LEDGER → /api/hub → Monitor compact-UI E2E — fs.appendFileSync to docs/META_LEDGER.md, /api/hub reflects new ledgerSummary, hub.refresh broadcast triggers re-render without crash — FX525, closes deep-audit CRITICAL "B191 bus→renderer fixture-only"). Remaining B199 gaps (B-B199-1..6) captured in BACKLOG for follow-on cycles: Brainstorm behavioral, Agents Replay/Genome behavioral, voice-substrate live (Whisper/Piper), cross-host install live, CI gate hardening, stub-only false-confidence specs.
- **B199 Phase 4 — Agents tab structural Playwright coverage** (`docs/plan-qor-b199-phase4-agents-e2e.md`). Establishes baseline coverage that the Agents top-level tab loads + 4 sub-pills (Operations/Timeline/Genome/Replay) render + active-pill switching works. Branched off Phase 3; linear chain. 3 new active FX520 Playwright cases. Deep per-sub-view behavioral coverage (agent health metrics, run list, genome failure patterns, replay step playback) explicitly deferred — one surface per phase discipline.
- **B199 Phase 3 — Integrations tab Playwright** (`docs/plan-qor-b199-phase3-integrations-e2e.md`). Closes the Voice Pack installed-state case that Phase 2 explicitly deferred pending the B-EM-4 `/api/hub` override substrate (now replicated inline on this branch). Branched off `feat/bicameral-enhancements-quickwins` to inherit B-BIC-1 + B-BIC-4. 1 new active FX519 Playwright case (Voice Pack installed state renders Uninstall + version) + 1 test.skip-staged (Bicameral capability-driven UI dimming, deferred until B-BIC-13 UI consumer ships). Bicameral ratify E2E intentionally not duplicated — already covered by FX490 in `integrations-bicameral.spec.ts`; B-BIC-1 ledger append covered by FX514 mocha. 2426 mocha pass / 17 baseline failing (zero regressions). B199 Phases 4-8 still outstanding (Brainstorm / Agents / Workspace / Risks / commands / routes / WebSockets / voice-live).
- **Bicameral integration quick wins (B-BIC-1..5)** (`docs/plan-qor-bicameral-quickwins.md`). Five high-leverage fixes against the v1 Bicameral MCP integration: (1) ratify decisions now anchor to META_LEDGER as `USER_OVERRIDE` entries with full decisionId/verdict/rationale payload; (2) extension-deactivate disposer terminates the stdio subprocess + `wireFromConfig` disconnects the prior client on rewire (no more orphans); (3) `transport.onclose` listener flips `isConnected` to false on subprocess crash so the next `callTool` throws cleanly; (4) `BicameralMcpClient.getCapabilities()` exposes the cached `listTools()` set, paving B-INT-1 incremental tool rollout; (5) install stdout/stderr sanitizer strips ANSI CSI sequences + C0 control characters before surfacing to UI/WebSocket (defensive against future bicameral CLI color output). 15 new SG-035 functional cases (FX514-FX518). 1-cycle architect-reviewer audit: PASS-WITH-FIXES (3 mechanical amendments to the plan — disposer typed surface, sanitizer line range, capability narrowing — all applied before implementation). Source: deep-review subagent surfaced 23 enhancements total; 18 remaining captured as B-BIC-6..23 for future cycles.
