...
const MANUAL_OVERRIDES = Object.freeze({
  ...
  FX141: { status: 'verified', reason: 'B sweep: sentinel.verdict event emission tested via sentinel/Engines.test.ts + sentinel/VerdictEngine.test.ts (generateVerdict produces dispatched verdicts, eventBus.emit asserted). Re-cited 2026-08-21: the prior citation named VerdictRouter.test.ts, which no longer exists.' },
  FX142: { status: 'verified', reason: 'B sweep: hub fanout to ws + EventBus emission tested via governance/PromptTransparency.test.ts + roadmap/transparency-events-hub.test.ts — all 4 event types flow through bus to subscribers. Re-cited 2026-08-21: the prior citation used a bare filename the resolver cannot locate.' },
