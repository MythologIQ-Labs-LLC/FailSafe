 ...
 * Node types include checkpoint / state / failure / governance. The graph is a
...
export interface GenomeNode { id: string; type: string; label: string; metadata?: Record<string, unknown> }
export interface GenomeEdge { id: string; source: string; target: string; type: string; metadata?: Record<string, unknown> }
export interface GenomeGraph { nodes: GenomeNode[]; edges: GenomeEdge[] }
...
export function parseGenomeGraph(json: string): GenomeGraph {
  ...
  const nodes: GenomeNode[] = nodesIn
    ...
    .map((n) => ({
      ...
      metadata: (n.metadata && typeof n.metadata === 'object') ? n.metadata as Record<string, unknown> : undefined,
    }));
