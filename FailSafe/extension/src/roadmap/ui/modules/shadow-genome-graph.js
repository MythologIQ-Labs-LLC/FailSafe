...
function nodeSvg(n, p, deg, sev, sel, nb) {
  ...
  const sevSuffix = isFail ? `, severity ${esc(sv || 'unclassified')}` : '';
  return `<g class="sg-node${dim}${n.id === sel ? ' sel' : ''}" transform="translate(${p.x.toFixed(1)},${p.y.toFixed(1)})" data-node="${esc(n.id)}" tabindex="0" role="button" aria-label="${esc(n.type)}: ${esc(n.label)}${sevSuffix}">${ring}${shape}${label}<title>${esc(n.label)} (${esc(n.type)})</title></g>`;
}
