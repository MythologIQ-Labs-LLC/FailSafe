...
function buildOptions(overrides: Partial<QoreRuntimeOptions> = {}): QoreRuntimeOptions {
  return {
    enabled: true,
    baseUrl: 'http://qore.test',
    timeoutMs: 1000,
    ...overrides,
  };
}
...
suite('QoreRuntimeService', () => {
  test('fetchJson resolves to {ok:true, body} on 200', async () => {
    ...
    assert.equal(calls[0].url, 'http://qore.test/health');
  ...
  test('fetchJson short-circuits with disabled error when enabled=false', async () => {
    ...
    const service = new QoreRuntimeService(buildOptions({ enabled: false }), fetchStub);
  ...
  test('proxy with enabled=false returns 503 + disabled message and skips fetch', async () => {
    ...
    assert.deepEqual(res.payload, { error: 'Qore runtime integration is disabled' });
  ...
  test('fetchSnapshot returns disabled snapshot when enabled=false', async () => {
    ...
    assert.equal(snap.baseUrl, 'http://qore.test');
  });
  ...
  test('fetchSnapshot returns connected=true with policyVersion when health + policy succeed', async () => {
    ...
    const service = new QoreRuntimeService(buildOptions(), buildFetchStub(responder, calls));
