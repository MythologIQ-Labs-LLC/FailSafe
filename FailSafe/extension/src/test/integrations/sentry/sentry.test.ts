...
suite('sentry-client (#102)', () => {
  test('buildIssuesPath defaults query to is:unresolved + scopes environment', () => {
    assert.match(buildIssuesPath('acme', 'backend'), /^\/api\/0\/projects\/acme\/backend\/issues\/\?query=is%3Aunresolved$/);
    assert.match(buildIssuesPath('acme', 'backend', { environment: 'production' }), /environment=production/);
  });
