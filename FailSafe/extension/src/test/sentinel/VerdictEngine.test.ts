...
suite('VerdictEngine (FX346)', () => {
  ...
  // FX930 (FailSafe#367 tranche 3a): generateVerdict's optional trailing
  ...
  test('FX930 generateVerdict — fileContent provided: verdict.artifactHash is its sha256', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
  ...
  test('FX930 generateVerdict — no fileContent: verdict.artifactHash stays undefined', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
    const v = await e.generateVerdict(EVT(), 'src/foo.ts', []);
  ...
  test('FX930 generateVerdict — same content hashes identically across two calls (determinism)', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
  ...
  test('FX930 generateVerdict — different content hashes differently', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
  ...
  test('FX930 generateVerdict — empty-string fileContent still hashes (distinct from "no content")', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
  ...
  test('FX930 executeActions — ledger entry carries the same artifactHash as the verdict', async () => {
    const s = makeStubs({ riskGrade: 'L3' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
  ...
  test('FX930 executeActions — no fileContent: ledger entry artifactHash is undefined', async () => {
    const s = makeStubs({ riskGrade: 'L1' });
    const e = new VerdictEngine(s.trust, s.policy, s.ledger, s.shadow);
    await e.generateVerdict(EVT(), 'src/x.ts', []);
