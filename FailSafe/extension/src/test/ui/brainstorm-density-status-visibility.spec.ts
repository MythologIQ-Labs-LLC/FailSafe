...
for (const [label, size] of Object.entries({
  ...
  test(`FX244 — density-status is visible and unoccluded at ${label}`, async ({ page }) => {
    ...
    // .cc-bs-chat has no live producer as of this audit (grep across
    // src/roadmap/ui/ finds only its CSS rule, no matching markup) — this
    // assertion is a forward guard, inert today, real the moment that
    // markup returns.
    const chatCount = await page.locator("#workspace .cc-bs-chat").count();
    if (chatCount > 0) {
