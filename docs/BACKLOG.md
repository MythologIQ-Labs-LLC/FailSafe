...
- [ ] **[B195] HIGH — VSIX size 47.6 MB approaches marketplace cap**: `mythologiq-failsafe-5.0.0.vsix` = 47,605,948 bytes (45.4 MB) vs `4.9.9.vsix` = 25,407,041 bytes (24.2 MB) — 1.87x bloat from vendored Piper (~63 MB unpacked) + Whisper (~20 MB) assets. VS Code Marketplace practical ceiling is ~50 MB; OpenVSX warns earlier. Future asset additions will breach. **Fix scope**: (a) postinstall fetch for Piper instead of bundle (extension downloads on first use); OR (b) git-LFS for binary assets (requires marketplace tooling that supports LFS expansion); OR (c) smaller voice model option as default with full set as opt-in download. **Origin**: qor-deep-audit Phase 1 GAP-CFG-01; previously flagged in `project_v4_10_1a_implementation_status.md` "63MB Piper vendor binaries committed" caveat. | v5.0.x or v5.0.0+
- [ ] **[B196] HIGH — FailSafe Pro daemon detection NOT implemented despite docs claim**: `PRIVATE/docs/LICENSING_POSTURE.md` states the extension "becomes thin client when Pro daemon detected"; code search returns zero implementations of daemon detection (`detectPro`, `proDaemon`, daemon socket/port probe — none exist). Only marketing URL constants in `src/shared/constants.ts:13-14` and Settings card link in `src/roadmap/ui/modules/settings.js:60-95`. **Fix scope**: either (a) implement detection (probe `localhost:<pro-port>` or named pipe; on success switch ConsoleServer into bridge mode and let Pro daemon serve hub data); OR (b) retract the docs claim until shipped. **Origin**: qor-deep-audit Phase 1 GAP-DEPS-09. | v5.0.0+ (gated on Pro launch)
