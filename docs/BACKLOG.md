...
| **v5.4.2**   | **Integration Expansion**      | 🚀 RELEASING   | Release-gate hotfix that pins the VS Code test version to last-known-good **1.122.1** (`.vscode-test.mjs` + `scripts/rebuild-vscode-electron.cjs`; overridable via `FAILSAFE_VSCODE_TEST_VERSION`), decoupling the release gate from VS Code stable's auto-update so an Electron bump can't dead-tag a release. Carries the full v5.4.0 integration + governance batch and the v5.4.1 stale-test fix. PRs #121–#135. **Follow-up filed:** run `test:all` on release-class PRs so native-build / UI-test regressions surface before the tag (both prior dead tags were invisible until the tag fired). |
