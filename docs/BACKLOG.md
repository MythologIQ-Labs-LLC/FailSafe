...
| **v5.6.3**   | **Governance bundle**          | 🚀 RELEASING   | Four governance features in one patch. **PR↔issue linkage governance** (#154, FX861 auditor + FX864 live-wiring) — a Check Run flags the `Closes #1, #2` footgun (GitHub closes only the first); pure auditor (9 tests) + `publishLinkageCheck` bridge + command glue (27 github-checks tests). **Tracker-as-governance-sidecar** (FX862) — `governance-projection.ts` projects a `TrackerManifest` from META_LEDGER + FEATURE_INDEX (governed-repo authoritative source; FX857 generator = ungoverned fallback); 6 tests + real-data smoke. **Shadow-genome consumer** (#118, FX863) — pure parse/summarize/governanceSubgraph + off-by-default `loadShadowGenome` over qor-logic's causal graph; 7 tests + real-Python smoke. Three views of the governance ledger (the convergence in the research brief). META_LEDGER #427–#430 (SUBSTANTIATE). Closes #118; #154 complete. |
