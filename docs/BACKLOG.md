...
- [ ] **[B-BIC-1] HIGH — META_LEDGER + USER_OVERRIDE on Bicameral ratify (C1 + E2)**: `BicameralRoute.ts:182-208` ratify handler does not append to META_LEDGER nor emit on EventBus. Ratify is the canonical "operator intentionally accepted X" event — must surface as `USER_OVERRIDE` ledger entry with `{kind:'bicameral.ratify', decisionId, verdict, rationale}`. **Fix scope**: one-line `ledger.appendEntry` in the success path of `ratify` POST + add optional rationale field to the row UI. | v5.2.x
- [ ] **[B-BIC-2] HIGH — `BicameralMcpClient.disconnect()` disposer registration (B3)**: `bootstrapBicameral.ts:42-58` never registers a `context.subscriptions.push({dispose: () => client?.disconnect()})`. On extension reload the stdio child is orphaned. **Fix scope**: 4 LOC disposer push. | v5.2.x
- [ ] **[B-BIC-3] HIGH — Transport close listener + crash recovery (B1)**: `BicameralMcpClient.ts:30-58` never subscribes to transport `close`/`exit` events. If bicameral-mcp crashes, `isConnected()` keeps returning true; subsequent `callTool` rejects opaquely. **Fix scope**: in `connect()`, attach `transport.onclose = () => { this.client = null; broadcast('bicameral.disconnected'); }`. | v5.2.x
- [ ] **[B-BIC-4] HIGH — Capability negotiation via `listTools()` (G2)**: `BicameralMcpClient.ts:54` calls `callTool` blind. Server without `bicameral.ratify` returns opaque 500 on UI click. After `connect()`, cache `client.listTools()` capability set; UI dims affordances by capability. Directly enables B-INT-1 incremental rollout. | v5.2.x
- [ ] **[B-BIC-5] MEDIUM — Install stdout/stderr sanitizer (A3)**: `install-handler.ts:114-122` forwards bicameral CLI stdout/stderr to UI + WebSocket with no ANSI/control-char stripping. `BicameralRoute.ts:88-91` broadcasts the same `stdoutTail`. **Fix scope**: `replace(/\x1b\[[0-9;]*m/g, '')` + strip C0 controls before emit + length cap. | v5.2.x

### Remaining 18 enhancements (deferred — pick up in follow-on cycles)

#### Security / Trust Boundary
- [ ] **[B-BIC-6] MEDIUM — Symlink bypass on `isSafeBicameralCommand` (A2)**: validator checks string path, not `realpath`. Add `fs.realpath()` + re-check containment at probe time. `install-detector.ts:30-37`. | v5.2.x or later
- [ ] **[B-BIC-7] MEDIUM — Windows path allowlist gap (A1)**: validator rejects legitimate scoop/chocolatey shims under `C:\ProgramData\…`. Add configurable allowlist of additional anchored absolute roots. `install-detector.ts:11,20-38`. | v5.2.x or later
- [ ] **[B-BIC-8] LOW — `connect()` concurrency race (A4)**: two callers both see `client=null`, leak first transport. Cache in-flight `Promise<void>` and return it on re-entry. `BicameralMcpClient.ts:43-58`. | v5.2.x or later

#### Lifecycle / Resilience
- [ ] **[B-BIC-9] MEDIUM — Idle disconnect TTL (B2)**: stdio subprocess sits at 30-60MB RSS indefinitely. Add `failsafe.integrations.bicameral.idleDisconnectMs` (default 15min), reconnect-on-demand. | v5.2.x or later
- [ ] **[B-BIC-10] MEDIUM — Config-change rebuild orphans prior subprocess (B4)**: `bootstrapBicameral.ts:38-39` overwrites client without disconnecting. Capture prior client, await `disconnect()` before replacement. | v5.2.x or later

#### Observability
- [ ] **[B-BIC-11] MEDIUM — Structured `isError` payload surfacing (C2)**: `BicameralMcpClient.ts:89-91` throws flat string. Surface `result.content[0].text` + include in broadcast event with decision-id context. | v5.2.x or later

#### UI / UX
- [ ] **[B-BIC-12] MEDIUM — "Open binding in editor" affordance on decision rows (D1)**: `bicameral-card.js:144-165` rows have title + binding + Ratify only. Add `data-action="bicameral-open-binding"` that posts `vscode.open` with filePath:startLine. | v5.2.x or later
- [ ] **[B-BIC-13] LOW — Capabilities-driven empty-state copy (D2)**: `bicameral-card.js:189-194` mentions `/bicameral-ingest` but `ingest` is unwired (B-INT-1). Gate hint by `capabilities.ingest === true` (consumes B-BIC-4 cache). | v5.2.x or later
- [ ] **[B-BIC-14] LOW — Composite Sync button (D3)**: header "Detect again" only re-probes status. When connected, compose status + history + drift fetches. | v5.2.x or later
- [ ] **[B-BIC-15] MEDIUM — Narrow-viewport overflow on decision rows (D4)**: at <800px the `<code>` filePath bindings push status badge + Ratify off-screen. Add `overflow:hidden;text-overflow:ellipsis;white-space:nowrap` to binding span. `bicameral-card.js:152-164`. | v5.2.x or later

#### Integration with FailSafe substrates
- [ ] **[B-BIC-16] HIGH — Drifted decisions → L3 approval auto-enqueue (E1)**: `drifted` status is exactly the semantic L3 approval serves. Enqueue L3 with `kind:'bicameral-drift-resolution'` linking decision id. | v5.2.x or later
- [ ] **[B-BIC-17] MEDIUM — Bicameral verdict events through SentinelWatchPolicy (E3)**: route bicameral verdict events through Sentinel for priority/notification classification (parallel to other FailSafe integrations). | v5.2.x or later
- [ ] **[B-BIC-18] MEDIUM — Risks Register mirror for drifted decisions (E4)**: mirror each `drifted` decision as `risks.json` entry keyed `bicameral:{decisionId}`; auto-close on ratify. | v5.2.x or later

#### Test coverage
- [ ] **[B-BIC-19] HIGH — Type-surface deferred tools (F1)**: Unwired tools (B-INT-1) have zero type surface in client — refactor risk. Add private `callRaw(name,args)` + TODO-comment anchored argument names for the 9-10 deferred tools. | v5.2.x or later
- [ ] **[B-BIC-20] HIGH — Live-subprocess MCP stdio test (F2)**: all `BicameralMcpClient.test.ts` cases stub transport. Add single integration test against vendored echo-server speaking MCP stdio framing (CI-gated). | v5.2.x or later
- [ ] **[B-BIC-21] MEDIUM — Concurrent connect/disconnect race tests (F3)**: per A4/B1, no test issues `Promise.all([connect(), connect()])` or interleaved connect+disconnect. | v5.2.x or later

#### Protocol awareness
- [ ] **[B-BIC-22] MEDIUM — MCP protocol version assertion (G1)**: `BicameralMcpClient.ts:54` constructs `Client({version:'1.0.0'})` (FailSafe identity, not protocol). After `connect()`, assert `getServerVersion()` + `getServerCapabilities()`; fail-closed below floor. | v5.2.x or later

#### Architectural debt
- [ ] **[B-BIC-23] MEDIUM — Runtime type guard on `callTool` return (H1)**: `BicameralMcpClient.ts:88` `as ToolCallResult` cast hides any SDK protocol shape change. Narrow with `isToolCallResult` runtime guard; reject otherwise. | v5.2.x or later
...
- [ ] **[B-EM-2] MEDIUM — Cross-session persistence for mode-transition history**: B194 V1 `ModeTransitionHistory` is in-memory only — transitions are lost on extension reload. Replay from META_LEDGER USER_OVERRIDE entries on initialize would restore the feed. **Fix scope**: at `ModeTransitionHistory` construction, query the ledger for the last 10 USER_OVERRIDE entries where `payload.action === 'governance_mode_changed' || 'break_glass_*'` and seed the ring. **Origin**: deferred from B194 V1. | v5.2.x or later
- [ ] **[B-EM-3] MEDIUM — First-run governance-mode onboarding wizard**: original B194 scope (deferred from V1). On first activation in a workspace with no prior mode set, launch a guided picker explaining observe vs assist vs enforce trade-offs and capturing the operator's deliberate choice. **Fix scope**: detect "first-run-no-mode" condition; show modal with three-option picker + explanatory copy. **Origin**: deferred from B194 V1. | v5.2.x or later
- [ ] **[B-EM-4] LOW — Playwright `serveConsoleServerUI` /api/hub override hook**: current harness injects hub via WebSocket `init` payload (consumed by Monitor compact UI's roadmap.js), but Command Center pages fetch `/api/hub` and the real HubSnapshotService output overwrites the init shape (no `recentModeTransitions`, no `governanceModeState` from test fakes). Extending the harness with an `/api/hub` override would let `initialHub` drive both surfaces, unblocking E2E coverage for FX509 governance-mode transitions, plus future feature work that lives on Command Center sub-tabs. **Fix scope**: add an Express middleware override in `serveConsoleServerUI` that intercepts `/api/hub` when `fixtures.initialHub` is set; tests opt in. **Origin**: surfaced during B194 Phase 4 implementation (2026-05-18); FX509 Playwright case `.skip`-staged pending this. | v5.2.x or later
