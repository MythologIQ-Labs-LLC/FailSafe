...
- [~] **[B-OD-8] (CONSERVATIVE SLICE staged on `feat/b-od-8-create-artifact-l3`; META_LEDGER #409; Review Boundary — not yet committed)** Open Design v1.2 — write-tool L3 gating. This cycle admits ONLY the non-destructive `create_artifact` via Buffer & auto-execute L3 (gate-by-construction one-shot token + enqueue route + per-item decide-l3 + `OpenDesignL3Executor` + L3-queue UI). FX806–FX811. **Deferred to a follow-up:** the 3 destructive tools (`write_file`/`delete_file`/`delete_project`) stay rejected — they warrant stricter per-tool thresholds. **Targets v5.3.3 (operator-set 2026-05-28; held — not committed).** | v5.3.3
...
| **v5.3.3**   | **Integration Beta**           | 🚧 RELEASING   | Open Design integration BETA — first write path: L3-gated `create_artifact` (Buffer & auto-execute, B-OD-8). Razor/clobber cleanup: B-INT-6 (`BicameralRoute.ts` 490→34), B-INT-7 (`bicameral-card.js` 314→98 + `MarketplaceRoute.ts` 382→29 + confirm-handler function-razor), B-INT-12 (TabGroup-level clobber guard, FX812). qor-debug transparency date-filter fix (FX813). META_LEDGER #409–#414. Merged to main via PR #116 (`520fdd0`); README beta announcement added. FX806–FX813. |
