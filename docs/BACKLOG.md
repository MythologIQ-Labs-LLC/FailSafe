...
- [ ] **[B-OD-2]** Open Design v1.1 — per-run SSE attach + daemon liveness probe (`127.0.0.1:7456` HTTP probe with TTL cache; enriches provenance with daemon-confirmed `projectId`; opt-in operator consent flow for SSE attach + replay). | v5.4.x or later
- [ ] **[B-OD-3]** Open Design v1.1 — vendored SSE event-type contracts with Apache-2.0 attribution; shipped under `src/integrations/open-design/contracts/chat-sse.ts`. | v5.4.x or later
