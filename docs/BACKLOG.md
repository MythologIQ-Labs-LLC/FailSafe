...
| **v5.5.1**   | **Maintenance**                | 📦 ON MAIN     | Maintenance patch (squash-merged to `main`; tag/publish deferred — operator publishes later). CI Actions bumped to their Node.js 24 runtimes, SHA-pinned (Dependabot #155–#158): `checkout` v4→v6.0.3, `setup-node` v4→v6.4.0, `upload-artifact` v4→v7.0.1, `download-artifact` v4→v8.0.1 (v8 fails closed on a digest mismatch). Development Tracker dashboard surfaces the server `{error}` body instead of a bare "HTTP 500". Issues #161 (Windsurf compatibility) + #154 (PR↔issue linkage governance) filed separately. |
