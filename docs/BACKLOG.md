...
| **v5.3.1**   | **Integration Surface (hotfix 1)** | 🚀 RELEASING | Updates `integrations-tab.test.ts` to assert v5.3.0's reality: 2 cards (Bicameral + Open Design) with positive assertions for both. Zero feature delta from v5.3.0 — the Open Design integration + WARN-only governance substrate ship verbatim. First v5.3.x build to actually reach the marketplaces. |
