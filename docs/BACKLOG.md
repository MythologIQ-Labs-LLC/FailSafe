...
| **v5.3.0**   | **Integration Surface**        | 🚀 RELEASING   | Open Design integration v1 (provenance) + v1.1 (MCP + SSE + probe) as second first-class MCP peer alongside Bicameral. WARN-only governance substrate v1 (secret scanning + FEATURE_INDEX verify + model-pinning lint). META_LEDGER #400 → #405 (documented Merkle fork at #401 per parallel-branch convention; first cycle to break the 4-cycle Plan-Time Hallucination remediation loop). |
