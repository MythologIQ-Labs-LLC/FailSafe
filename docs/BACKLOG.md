...
- [ ] [B-INT-6] Decompose `BicameralRoute.ts` (490 lines — over the 250-line Section-4 razor). Pre-existing debt surfaced + filed during the v5.1.8 B-INT-1 cycle (`plan-qor-v5-1-8-cycle.md` §Razor strategy). The B-INT-1 routes were isolated into `bicameralToolRoutes.ts` to avoid worsening it; the existing `status`/`install`/`connect`/`disconnect`/`history`/`drift`/`ratify`/`open-binding`/`auto-connect`/`upstream` route bodies still need extracting. | v5.2.x or later
- [ ] [B-INT-7] Decompose `bicameral-card.js` (314 lines — over the 250-line Section-4 razor). Pre-existing debt surfaced + filed during the v5.1.8 B-INT-1 cycle. The Advanced-tools section was isolated into the `bicameral-advanced-tools.js` leaf to avoid worsening it; the card's existing render blocks still need extracting. Audit advisory A4 also flags `MarketplaceRoute.ts` (382 lines) for the same treatment. | v5.2.x or later
- [ ] [B-INT-12] Promote the TabGroup inactive-sub-view render guard from a per-renderer opt-in to a TabGroup-level concern. `TabGroup.onEvent` (`tab-group.js`) fans events to ALL sub-views, but every sub-view shares one reused `contentEl` and only the active one's `.container` is reassigned in `renderActive()`. Any sub-renderer whose `onEvent` calls `render()`/`_setState` (writing `container.innerHTML`) while a SIBLING in the same group is active will clobber the visible pane. B-INT-5 (qor-debug 2026-05-28) fixed this for `BicameralRenderer` via an additive `_tgMounted` flag + early-return in `render()`, but the SAME latent clobber remains pre-existing (since B198 TabGroup consolidation) in: `timeline.js:119` · `genome.js:106` · `replay.js:189` (agents group), `risks.js:157` · `governance.js:280` (governance group; partially self-guarded by an `if (logEl)` DOM-presence check), `skills.js:300` (workspace group). Universal options: (a) `TabGroup.onEvent` fans only to the active sub-view (changes state-freshness semantics — inactive renderers must refetch on re-activation), or (b) every TabGroup-hosted renderer honors `_tgMounted`. Architectural — route through /qor-plan; verify each render() guards `this.container` before any container-null approach. | v5.3.x+
...
- [ ] **[B-OD-8]** Open Design v1.2 — write-tool L3 gating. Admit `create_artifact`, `write_file`, `delete_file`, `delete_project` with explicit L3 approval per call; the 3 destructive tools (`write_file`, `delete_file`, `delete_project`) warrant stricter trust thresholds than `create_artifact`. Highest-value extension after the read-only v1.1 foundation lands. | v5.4.x or later
...
| **v5.3.2**   | **Integration Surface (refactor)** | 🚧 PR OPEN     | Internal-quality release bundling B-INT-4 (shared `McpClientHost` substrate; Bicameral 291→188 LoC, Open Design 185→91 LoC) + B-INT-5 (Integrations tab → `TabGroup` sub-tab switcher; `IntegrationsRenderer` split into `BicameralRenderer` + `OpenDesignRenderer`). qor-debug caught + fixed one event-routing regression (FX804 `_tgMounted` clobber guard, test-first). B-INT-12 filed for the pre-existing 6-renderer clobber. META_LEDGER #407 + #408. No marketplace-feature change beyond the tab layout. |
