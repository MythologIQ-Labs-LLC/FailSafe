...
- [ ] [B-INT-8] Normalize the future-integration research packet into repository-ready contract reviews. Source: `docs/research/FailSafe Integrations Research.md`; target deliverables are the named `docs/research/integrations/INTEGRATION_*_CONTRACT_REVIEW.md` files listed in that packet. Blocks implementation of #95-#102 and #104-#108 until each issue has an explicit ready/conditional/research-first disposition. | v5.3.x+
- [ ] [B-INT-9] First integration tranche after research gate: #96 GitHub App checks, #97 Linear read-only issue import, #99 Semgrep/SARIF ingest, #100 Slack notify-only, #104 Continue headless wrapper, #107 Aider diff gate, and #108 MCP Registry read-only scoring. Shared constraints: disabled by default, no network unless enabled, secrets stored only in host secret storage, and fixture tests for every parsed vendor payload. | v5.3.x+
- [ ] [B-INT-10] Second integration tranche: #98 Jira read path, #101 Microsoft Teams notify-only, #102 Sentry read-only regression correlation, and #103 OpenTelemetry evidence export/import lane. Jira webhook renewal, Teams workflow ownership, Sentry release discipline, and OpenTelemetry semantic-convention stability remain conditional planning inputs. | v5.3.x+
- [ ] [B-INT-11] Research-first integration track: #95 Open Design daemon-vs-MCP path decision, #105 OpenHands start-of-run policy adapter, and #106 Cline/Roo/Kilo config-audit package. Do not plan write surfaces, remote approvals, or runtime interception for these until the contract gaps in `docs/research/FailSafe Integrations Research.md` are resolved. | v5.3.x+
