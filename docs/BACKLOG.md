...
- [x] [B132] **(v5.1.8 — Complete via `plan-qor-v5-1-8-cycle.md` Phase 2 — actual location was `BrainstormRoute.ts`, not `ConsoleServer.ts`; additive `labelTruncated`/`labelOriginalLength` response fields + `.bs-truncation-notice` inline UI; FX585/FX590)** Long node labels silently truncated server-side: `ConsoleServer.ts` `.slice(0, 200)` with no client feedback — user's full text accepted but shortened without notice | v4.6.0
...
- [x] [B-INT-1] **(v5.1.8 — Complete via `plan-qor-v5-1-8-cycle.md` Phase 4 — 11 `POST /api/actions/bicameral-<tool>` routes via the new `bicameralToolRoutes.ts` factory + a collapsible "Advanced tools" card section; the client surface was already done by B-BIC-19; FX586–FX589)** Surface remaining Bicameral tools (`ingest`, `search`, `brief`, `judge_gaps`, `resolve_compliance`, `link_commit`, `update`, `reset`, `dashboard`, `validate_symbols`, `get_neighbors`) — extends `BicameralMcpClient` + adds routes + UI affordances. Out of scope for v1 (would require dedicated UI surfaces beyond the current Integrations tab card). | v5.2.x or later
...
- [x] **[B-INT-4] (v5.3.2 — RELEASED; META_LEDGER #407)** Generic `McpClientHost` substrate extracted; Bicameral 291→188 LoC, Open Design 185→91 LoC; consolidated idle-scheduler. FX800/FX801.
- [x] **[B-INT-5] (v5.3.2 — RELEASED; META_LEDGER #408)** Integrations tab → `TabGroup` sub-tab switcher; `IntegrationsRenderer` split into `BicameralRenderer` + `OpenDesignRenderer` (`integrations.js` deleted). qor-debug fixed the FX804 `_tgMounted` clobber. FX802/FX803/FX804.
- [x] [B-INT-8] (2026-06-02) Normalized the future-integration research packet into repository-ready contract reviews. All 13 `docs/research/integrations/INTEGRATION_*_CONTRACT_REVIEW.md` files extracted, each with an explicit disposition: **Ready** — #97 Linear, #99 Semgrep/SARIF, #100 Slack (notify-only), #101 Teams (notify-only), #104 Continue (version-pin), #108 MCP Registry (read-only); **Conditional** — #96 GitHub (org App), #98 Jira (app reg + webhook renewal), #102 Sentry (release discipline), #105 OpenHands (start-of-run only), #107 Aider (auto-commit suppression); #106 Cline/Roo/Kilo (config-audit first, + Agent_Sudo prior-art); #95 Open Design (research-gate RESOLVED — dual-path + L3 write path already shipped v5.3.3). Gate cleared for B-INT-9/10/11. Remaining: register each disposition on its GitHub issue (operator-approval pending). | v5.3.x+
- [ ] [B-INT-9] (UNBLOCKED 2026-06-02 — B-INT-8 research gate cleared) First integration tranche: #96 GitHub App checks, #97 Linear read-only issue import, #99 Semgrep/SARIF ingest, #100 Slack notify-only, #104 Continue headless wrapper, #107 Aider diff gate, and #108 MCP Registry read-only scoring. Per-issue contract reviews under `docs/research/integrations/`. Shared constraints: disabled by default, no network unless enabled, secrets stored only in host secret storage, and fixture tests for every parsed vendor payload. | v5.3.x+
- [ ] [B-INT-10] (UNBLOCKED 2026-06-02 — B-INT-8 research gate cleared) Second integration tranche: #98 Jira read path, #101 Microsoft Teams notify-only, #102 Sentry read-only regression correlation, and #103 OpenTelemetry evidence export/import lane. Conditional planning inputs (now documented in the contract reviews): Jira webhook renewal, Teams workflow ownership, Sentry release discipline, OpenTelemetry semantic-convention stability. | v5.3.x+
- [ ] [B-INT-11] (research gate RESOLVED 2026-06-02) Research-first integration track: #95 Open Design (path decision RESOLVED — dual-path + L3 write path shipped v5.3.3, see B-OD-* / contract review; #95 ready to close or relabel), #105 OpenHands start-of-run policy adapter (conditional — start-of-run gating only), #106 Cline/Roo/Kilo config-audit package (config-audit-first; IDE interception deferred). Contract gaps resolved in `docs/research/integrations/`. | v5.3.x+
- [x] **[B-INT-6] (RELEASED v5.3.3 — merged PR #116 `520fdd0`; published VS Code Marketplace + Open VSX via tag `v5.3.3`; META_LEDGER #410 + DELIVER #415)** Decomposed `BicameralRoute.ts` 490 → 34 LoC. Extracted into `bicameralRouteShared.ts` (173 — deps contract + `governToolCall` + drift/parse helpers), `bicameralLifecycleRoutes.ts` (144 — status/install/connect/disconnect/auto-connect/upstream), `bicameralDecisionRoutes.ts` (181 — history/drift/ratify/open-binding); orchestrator re-exports `setupBicameralRoutes`/`BicameralRouteDeps`/`governToolCall` (importers + 6 test files unchanged); `bicameralToolRoutes.ts` repointed at the shared core (breaks the prior import cycle). All 4 modules ≤ 250. Zero behavioral change — 195 Bicameral mocha + 7 Playwright pass verbatim; eslint 0. | v5.3.3
- [x] **[B-INT-7] (RELEASED v5.3.3 — merged PR #116 `520fdd0`; published VS Code Marketplace + Open VSX via tag `v5.3.3`; META_LEDGER #411/#413 + DELIVER #415)** Decomposed both razor-over files. `bicameral-card.js` 314 → 98 LoC (extracted state renderers to `bicameral-card-render.js`, 232). `MarketplaceRoute.ts` 382 → 29 LoC (extracted `marketplaceRouteShared.ts` 37 [deps + HITL nonce store], `marketplaceReadRoutes.ts` 51, `marketplaceInstallRoutes.ts` 199, `marketplaceScanRoutes.ts` 105). All ≤ 250; public surfaces (`renderBicameralCard`/`bindBicameralCard`/`INITIAL_BICAMERAL_STATE`; `setupMarketplaceRoutes`/`MarketplaceRouteDeps`) preserved — importers + tests unchanged. Zero behavioral change — 90 mocha + 10 Playwright (7 bicameral + 3 marketplace) pass verbatim; eslint 0. **Residual CLEARED (META_LEDGER #413):** the `install/:id/confirm` completion was decomposed into `handleInstallCompletion`/`runPostInstallScan`/`recordInstallLedger` (≤25 LoC each) — function-razor now satisfied; 58 marketplace tests verbatim. | v5.3.3
- [x] **[B-INT-12] (RELEASED v5.3.3 — merged PR #116 `520fdd0`; published VS Code Marketplace + Open VSX via tag `v5.3.3`; META_LEDGER #412 + DELIVER #415)** Promoted the inactive-sub-view clobber guard to a TabGroup-level concern. Chosen design (over fan-only-to-active / per-renderer-_tgMounted): `TabGroup.renderActive` gives every INACTIVE sub-view a persistent **detached scratch `<div>`** (`_tgDetached`); only the active sub-view's `container` is the shared `contentEl`. Inactive event-driven renders write off-DOM; reactivation rebuilds the live pane. One-place change, zero per-renderer edits, behavior-preserving for the active sub-view; `_tgMounted` retained. Independent architect-reviewer audit PASS (all 11 sub-views + hidden-hazard layout pass). FX812 (5 jsdom cases incl. the Transparency cache-node pattern). Verified: 29 TabGroup + 481 sub-view mocha + 14 Playwright (4 tab groups) verbatim; tsc/eslint 0. **Note:** pre-existing, unrelated date-sensitive failures in `transparency-renderer.test.ts` tests 1–2 (red on baseline; NOT caused by B-INT-12) — candidate follow-up. | v5.3.3
- [ ] [B-INT-13] (added 2026-06-02) Mermaid Chart MCP integration — candidate on `docs/INTEGRATIONS.md`. MCP server `validate_and_render_mermaid_diagram` to render + VALIDATE governance diagrams (SHIELD lifecycle, Bicameral decision graph, the Development Tracker Convergence/Sequence sections) as lint-checked artifacts. Govern via `McpClientHost` (read-only allowlist, no write tools); admit via MCP-Registry scoring (#108). Do a contract review (`docs/research/integrations/INTEGRATION_MERMAID_MCP_CONTRACT_REVIEW.md`) before wiring, per the B-INT-8 research-gate discipline. | v5.4.x+
- [ ] [B-INT-14] (added 2026-06-02) Context7 MCP integration — candidate on `docs/INTEGRATIONS.md`. MCP server `resolve-library-id` + `query-docs` for current library/SDK docs; powers `/qor-research` + plan-time external-name verification (`feedback_verify_external_names_at_plan_time`) — verify external API names against live docs before a plan reaches `/qor-audit`, closing the ghost-helper/stale-API VETO class at source. Govern via `McpClientHost` (read-only); **data-minimization redaction guard mandatory** (queries carry only a library id + topic, never repo content/secrets); outbound egress disclosed + opt-in. Contract review before wiring. | v5.4.x+
- [ ] **[B-INT-15] (added 2026-06-04; deferred from #172 Part 2 "B2")** ACP enforce-proxy — deep fs/terminal **content/argv policy**. The proxy (shipped v5.6.0, GH #172) today governs an effect's *existence + path + scope + intent* through the real `EnforcementEngine` (an out-of-scope fs-write or a no-intent action is blocked); the action's *payload* (file content digest, terminal command + argv) is recorded for provenance but NOT policy-matched. Closing this means widening the shared `ProposedAction` (currently a closed `file_write|file_create|file_delete|file_rename` union with no payload field) to carry the digested payload, threading it through `contractMappers.evaluationRequestToProposedAction`, and authoring a content/argv policy (e.g. catastrophic-pattern terminal denylist, fs secret-pattern guard). **Own governed cycle** — touches core enforcement used everywhere; requires adversarial security review (NIST/OWASP/EU AI Act, as the proxy foundation did). Alternative design to weigh: a proxy-LOCAL content-policy layer (in the `acp/proxy` subtree, off the shared engine) to contain blast radius. | v5.7.x+

### qor.scripts Substrate Modules (plan-qor-substrate-modules-v1, 2026-05-27)

Follow-up work surfaced during substrate v1 integration. v1 ships 3 modules (secret_scanner, feature_index_verify TS adapter, model_pinning_lint) under WARN-only posture invoked via `failsafe.substrate.run` Command Palette entry. See `docs/SUBSTRATE_MODULES.md` for the v1 surface.

- [x] **[B-SUBSTRATE-1] (v5.3.0-baseline — IMPLEMENTED 2026-05-27 via plan-qor-substrate-modules-v1)** v1 substrate runner + 3 module wrappers + VS Code command + transparency-stream integration. FX710-FX715 (28 mocha cases + 4 `node --test` cases = 32 total). `'substrate.run.complete'` added to `FailSafeEventType` union; WARN-only posture; operator-triggered only. | v5.3.0
- [ ] **[B-SUBSTRATE-2]** `dependency_admission_lint` v2 — upstream module is Python-archetype-specific (reads `pyproject.toml`/`requirements*.txt`). FailSafe is a Node/TS extension. Either npm-port the check (FailSafe-local TS implementation reading `package.json`/lockfile) OR contribute upstream cross-archetype support. | v5.3.x+
- [ ] **[B-SUBSTRATE-3]** Auto-hook substrate runner into `/qor-substantiate` execution. v1 is operator-triggered via Command Palette only. Two paths: FailSafe extension intercept on the substantiate skill invocation, OR upstream qor-substantiate skill modification. | v5.3.x+
- [ ] **[B-SUBSTRATE-4]** Console UI panel for substrate run history + per-finding drill-down. v1 surfaces findings via VS Code Output channel + transparency stream event only; a persistent UI panel would let operators browse historical runs and click into individual finding records. | v5.3.x+
- [ ] **[B-SUBSTRATE-5]** SBOM emit integration (Phase 108 substrate concern, deferred from v1 batch). Out of scope for the current 4-module batch but tracked here for visibility. | v5.4.x+
- [ ] **[B-SUBSTRATE-6]** Skill-frontmatter backfill so `model_pinning_lint` produces meaningful coverage against `.claude/skills/`. v1 documents the silent-no-op behavior in `summary.note`; this item closes the underlying gap. Separate `qor-logic install` config concern. | v5.4.x+
...
## Open Design integration (v5.3.0)

- [x] **[B-OD-1] (v5.3.0 — IMPLEMENTED on `feat/open-design-integration`)** Open Design provenance attribution v1 (Model 2 — file-path-based). New `src/integrations/open-design/` (provenance extractor, detector class, barrel); new `src/sentinel/IAgentProvenanceDetector.ts`; `AgentRunRecorder` constructor gains optional `{ provenanceDetectors }` options-bag + new `attachProvenance(runId, provenance)` method; `handleFileEdit` runs detectors once per filePath and attaches first non-null result to every active run lacking provenance; Monitor Agents → Replay sub-view renders an "Open Design" origin pill when `run.provenance.source === 'open-design'`; opt-in via `failsafe.integrations.openDesign.enabled` (default false, requires extension reload to take effect). FX700-FX705 (28 cases — 7 + 7 + 4 + 5 + 2 Playwright + 3 node:test). Plan: `plan-open-design-integration-v1.md` (v3). Audit: PASS (v1 + v2 VETO → v3 PASS). | v5.3.0
- [x] **[B-OD-2] (v5.3.x — Closed by B-OD-7)** Open Design v1.1 — per-run SSE attach + daemon liveness probe. Shipped as `OpenDesignSseClient` + `OpenDesignDaemonProbe` (127.0.0.1:7456 HTTP probe with 30s TTL cache). Opt-in via `failsafe.integrations.openDesign.sseEnabled` setting. | v5.3.x
- [x] **[B-OD-3] (v5.3.x — Closed by B-OD-7)** Open Design v1.1 — vendored SSE event-type contracts with Apache-2.0 attribution. Shipped under `src/integrations/open-design/contracts/sse-chat.ts` (filename normalized vs original `chat-sse.ts` proposal) + `contracts/NOTICE.md`. | v5.3.x
- [ ] **[B-OD-4]** Open Design v2 — FailSafe-as-Open-Design-plugin (upstream publish). Out of scope for v1; requires upstream PR + maintainer coordination. | v6.0.x or later
- [ ] **[B-OD-5]** Open Design — skill auto-select upstream PR. Documents the discovery-vs-activation caveat in `docs/INTEGRATIONS.md` (Open Design section) and tracks the upstream change to wire qor-* skill auto-activation when FailSafe is present in the workspace. | v6.0.x or later
- [x] **[B-OD-7] (v5.3.x — IMPLEMENTED on `feat/open-design-integration-v1.1`)** Open Design v1.1 — MCP adapter + per-run SSE attach + daemon-liveness probe. New `src/integrations/open-design/`: `OpenDesignDaemonProbe`, `OpenDesignMcpClient` (transplanted from `BicameralMcpClient.ts:67-180` pattern with local `idle-scheduler.ts` duplicate-by-design), `OpenDesignSseClient`, `OpenDesignMcpAllowlist` (7 read + 4 write tools back-cited to upstream `apps/daemon/src/mcp.ts@abe72af` line numbers), vendored `ChatSseEvent` discriminated union with Apache-2.0 attribution. New `src/extension/bootstrapOpenDesignMcp.ts` wires command `failsafe.openDesign.registerMcp` + opt-in settings `failsafe.integrations.openDesign.mcpEnabled` + `failsafe.integrations.openDesign.sseEnabled`. v1.1 invariant: write tools REJECTED at runtime with `WRITE_TOOL_NOT_ENABLED` (deferred to B-OD-8). FX720-FX725 (60 cases — 9 + 7 + 10 + 6 + 7 + 7 + cross-coverage). Plan: `plan-open-design-integration-v1.1.md` (v3 PASS verdict, closes 4-cycle Plan-Time Hallucination remediation loop). | v5.3.x
- [x] **[B-OD-8] (RELEASED v5.3.3 — merged PR #116 `520fdd0`; published VS Code Marketplace + Open VSX via tag `v5.3.3`; META_LEDGER #409 + DELIVER #415)** Open Design v1.2 — write-tool L3 gating. This cycle admits ONLY the non-destructive `create_artifact` via Buffer & auto-execute L3 (gate-by-construction one-shot token + enqueue route + per-item decide-l3 + `OpenDesignL3Executor` + L3-queue UI). FX806–FX811. **Deferred to a follow-up (see [B-OD-12]):** the 3 destructive tools (`write_file`/`delete_file`/`delete_project`) stay rejected — they warrant stricter per-tool thresholds. | v5.3.3
- [ ] **[B-OD-9]** Open Design v1.2 — AG-UI stream consumption (`/api/runs/:id/agui`) for richer in-flight observability beyond ChatSseEvent. | v5.4.x or later
- [ ] **[B-OD-10]** Open Design v1.2 — multi-daemon support (OD + FailSafe Pro daemon coexistence; discovery via configurable URL list rather than hardcoded `127.0.0.1:7456`). | v5.4.x or later
- [ ] **[B-OD-11]** (defensive) Upstream Open Design PR proposing auto-select for governance-tagged skills (per Entry #399 Q4 caveat about discovery-vs-activation). | v6.0.x or later
- [ ] **[B-OD-12]** Open Design v1.2 — destructive write-tool tranche (`write_file`/`delete_file`/`delete_project`). Split out from B-OD-8 (which admitted only the non-destructive `create_artifact`). Each destructive tool warrants a stricter per-tool L3 threshold (e.g., diff-preview + path-scope confirmation for `write_file`; explicit double-confirm for the two `delete_*` tools) rather than reusing the `create_artifact` Buffer & auto-execute flow verbatim. Until shipped, these stay rejected at runtime with `WRITE_TOOL_NOT_ENABLED`. | v5.4.x or later

## Deferred follow-ups (filed during v5.3.3 DELIVER close-out, 2026-05-28)

- [ ] **[B-BIC-24]** De-flake the Bicameral idle-disconnect TTL test. `BicameralMcpClient.idleDisconnect.test.js:49` uses real timers and intermittently fails (`true !== false` race) under CI load — it failed the v5.3.3 Release Pipeline once and was cleared only by `gh run rerun --failed` (last touched commit `5691d3a`, v5.1.6; unrelated to v5.3.3 content). Rewrite against fake timers (or poll-with-timeout on the disconnect assertion) so the idle-disconnect behavior is verified deterministically rather than on a wall-clock race. A flaky test in the release gate is a recurring publish hazard. | v5.4.x or later
- [ ] **[B201]** Transparency renderer `entry.time` no-`Z` fallback normalization (renumbered from a duplicate B200 during v5.4.0 release prep). qor-debug #414 residual: `matchesFilter` now compares epochs via `Date.parse`, but an `entry.time` value lacking a `Z`/offset suffix is parsed as **local**, not UTC — a low-severity correctness gap deferred in #414 because the fix touches the `eventKey` dedup surface (changing the parsed value risks shifting dedup keys for in-flight streams). Normalize bare timestamps to UTC at ingest (or stamp `Z` at emit) and re-verify the dedup key is stable. Source: `FailSafe/extension/src/roadmap/ui/modules/transparency.js:93`. | v5.4.x or later

## Qor-logic consumer-adapter scheduled migrations (#233, filed 2026-07-13)

The v1 consumer adapter (`FailSafe/extension/src/qorlogic/consumer/`, FX892/FX893) classifies the four file-based artifacts FailSafe provably consumes (META_LEDGER, FEATURE_INDEX, programs.yaml, audit-gate JSON). Issue #233 permits the remaining consumers to be "migrated or explicitly scheduled" — these rows are that schedule.

- [ ] [B202] Consumer-adapter migration: shadow-genome client — route `shadow-genome-client.ts` `parseGenomeGraph()` (Python-subprocess genome; empty-on-bad-input + feature-presence fallback at :76-81) through an `ArtifactEnvelope`-returning reader so a corrupt or absent `.qor/genome.jsonl` is distinguishable from an honestly empty genome. | v5.10.x+
- [ ] [B203] Consumer-adapter migration: tracker projection sources — `governance-projection.ts` and `TrackerRoute.ts` read META_LEDGER / FEATURE_INDEX / programs.yaml directly; consume the adapter envelopes so tracker projections surface malformed/stale source state instead of silently projecting from valid-empties. | v5.10.x+
- [ ] [B204] Consumer-adapter migration: GovernancePhaseTracker parser unification — collapse the alternate `GovernancePhaseTracker.parseMetaLedger()` onto the canonical `parseMetaLedgerEntries()` model behind the adapter (different output contract; explicitly excluded from the #233 v1 cycle per plan boundaries), then route `WorkspaceArtifactBuilder.readGovernanceState()` through `readMetaLedgerArtifact`. | v5.10.x+

## Section 4 razor debt (filed 2026-07-13, Wave-1 stabilization cycles)

- [x] [B205] HubSnapshotService Section 4 decomposition (v5.10.x — Complete) — `hub-payload-assembler.ts` owns pure composition/coalescing, `hub-revert-deps.ts` owns revert construction, and the retained service/public payload seam passes the binding 250/40/3 Razor. | v5.10.x
- [x] [B208] Pro download-URL drift ruling (v5.10.x — Complete) — canonical current route is `https://mythologiq.studio/products/failsafe-download`; ROADMAP, SYSTEM_STATE, historical-plan supersession note, shipped constant, README, and refreshed architecture baseline now agree. | v5.10.x
- [ ] [B207] AHP (Agent Host Protocol) adapter-strategy watchlist — microsoft/agent-host-protocol went public (MIT; spec + schemas + Rust/TS/Kotlin/Go/Swift client libs; observed 2026-07-13); VS Code 1.126 rearchitected agent sessions onto AHP and remote Agents-window sessions run AHP over SSH/dev-tunnel. Protocol-level governance seam parallel to the ACP enforce-proxy (#172/GH #161 lineage): an AHP client could observe/govern agent sessions without any Agents-window extension support. Evaluate an AHP observer/enforce adapter when the spec tags a stable release or #83's hold clears. See docs/VS_CODE_AGENTS_WINDOW.md §7. | post-v5.10.x
- [ ] [B206] Legacy test-file razor debt — `brainstorm-graph.test.ts` (299), `brainstorm-tab.spec.ts` (282), and `brainstorm-route.test.ts` (354) remain the open legacy split. The 2026-07-13 extension was completed: `install-skills-card.test.ts` and `voice-pack-install.test.ts` are now below 250 lines with status/guard/helper sibling suites, verified by the binding Razor. | v5.10.x+

## Wishlist (Nice to Have)

- [ ] [B90] CLI Overseer Lite Feasibility Extraction: Explore creating a lightweight, CLI-compatible version of FailSafe for direct website integration | v5.4.x+ _(re-tagged 2026-06-02 from stale v5.0.0 — shipped without this exploratory item)_
- [ ] [B91] Formal Security Hardening Roadmap Document: Define the 3-phase security implementation plan (Foundation, Advanced, Resilience) with milestone criteria and a dependency graph | v5.4.x+ _(re-tagged 2026-06-02 from stale v5.0.0 — overlaps roadmap M9)_
...
| **v5.1.8**   | **Bicameral Tool Surface**     | ✅ RELEASED    | Bicameral Advanced-tools (B-INT-1: 11 tool routes + card section), Sentinel/Governance mode UI fix (B-EM-1), B132 truncation feedback, B199 epic closeout |
| **v5.2.0**   | **FailSafe Learn**             | ⚠️ TAGGED-NOT-PUBLISHED | Release Pipeline failed at Build & Test (5 unit-test failures from orphaned SHIELD lesson anchors + FX615 tag-filter race); marketplace publish jobs skipped. Superseded by v5.2.1. Git tag points at `ba9a927` in main's history. |
| **v5.2.1**   | **FailSafe Learn (hotfix 1)**  | ⚠️ TAGGED-NOT-PUBLISHED | Release Pipeline failed at Build & Test — latent `popout-ui.spec.ts` Playwright regression surfaced once v5.2.1's unit-test fix let CI reach the Playwright phase for the first time in the v5.2.x line. Marketplace publish jobs skipped. Superseded by v5.2.2. Git tag points at `7631ac1` in main's history. |
| **v5.2.2**   | **FailSafe Learn (hotfix 2)**  | ✅ RELEASED    | First v5.2.x build to actually publish to VS Code Marketplace + Open VSX. Zero feature delta from v5.2.0/v5.2.1. Migrates `popout-ui.spec.ts` to the modern `serveConsoleServerUI` harness used by every other v5.2.0+ Playwright spec. META_LEDGER #396. |
| **v5.3.0**   | **Integration Surface**        | ⚠️ TAGGED-NOT-PUBLISHED | Release Pipeline failed at Build & Test — `integrations-tab.test.ts:34` hardcoded `cards.length === 1` ("Bicameral is the only card in v1") became outdated when v5.3.0 added the Open Design Settings card. Marketplace publish jobs skipped. Superseded by v5.3.1. Git tag points at `08916d9` in main's history. |
| **v5.3.1**   | **Integration Surface (hotfix 1)** | ✅ RELEASED    | First v5.3.x build to actually publish to VS Code Marketplace + Open VSX. Zero feature delta from v5.3.0 — the Open Design integration + WARN-only governance substrate ship verbatim. Updates `integrations-tab.test.ts` to assert v5.3.0's reality (2 cards: Bicameral + Open Design) plus a follow-up `transparency-renderer` timestamp test fix (PR #114). META_LEDGER #406. |
| **v5.3.2**   | **Integration Surface (refactor)** | ✅ RELEASED    | Internal-quality release bundling B-INT-4 (shared `McpClientHost` substrate; Bicameral 291→188 LoC, Open Design 185→91 LoC) + B-INT-5 (Integrations tab → `TabGroup` sub-tab switcher; `IntegrationsRenderer` split into `BicameralRenderer` + `OpenDesignRenderer`). qor-debug caught + fixed one event-routing regression (FX804 `_tgMounted` clobber guard, test-first). B-INT-12 filed for the pre-existing 6-renderer clobber. META_LEDGER #407 + #408. Published to VS Code Marketplace + Open VSX via PR #115 → tag `v5.3.2` (admin-merge; production gate approved). No marketplace-feature change beyond the tab layout. |
| **v5.3.3**   | **Integration Beta**           | ✅ RELEASED    | Open Design integration BETA — first write path: L3-gated `create_artifact` (Buffer & auto-execute, B-OD-8). Razor/clobber cleanup: B-INT-6 (`BicameralRoute.ts` 490→34), B-INT-7 (`bicameral-card.js` 314→98 + `MarketplaceRoute.ts` 382→29 + confirm-handler function-razor), B-INT-12 (TabGroup-level clobber guard, FX812). qor-debug transparency date-filter fix (FX813). META_LEDGER #409–#414 + DELIVER #415. Merged to main via PR #116 (`520fdd0`) + release bump PR #117 (`d56197b`); tag `v5.3.3` (`d3a0b75`). Published to Open VSX via CI and to VS Code Marketplace via local `vsce publish --azure-credential` (CI marketplace job hit the documented `/_apis/gallery` PAT-auth timeout; identical CI-built artifact). README beta announcement added. FX806–FX813. |
| **v5.4.0**   | **Integration Expansion**      | ⚰️ DEAD TAG    | Tag `v5.4.0` (`1563c80`) pushed but its Release Pipeline failed at `npm run test:all` → both publish jobs **skipped**; never reached either marketplace. Cause: the B-INT-5 Integrations sub-tab Playwright test asserted 2 sub-view pills, but the new MCP Catalog tab (B-INT-13/14) made it 3. Superseded by **v5.4.1** (test hotfix; same feature batch). Tag left in history per the v5.2.0/v5.2.1 forward-only dead-tag precedent. |
| **v5.4.1**   | **Integration Expansion**      | ⚰️ DEAD TAG    | Tag `v5.4.1` (`02a7d8f`) pushed; fixed the v5.4.0 stale pill test, but its Release Pipeline failed at the `better-sqlite3` native rebuild → both publish jobs **skipped**; never reached either marketplace. Cause was **environment drift, not code**: VS Code stable bumped 1.122.1 → 1.123.0 between the v5.4.0 (06:51) and v5.4.1 (13:17) runs; the newer Electron (V8 13.x) removed APIs (`v8::External::Value()`, `PropertyCallbackInfo::This()`) used by `better-sqlite3@12.6.2`. The test gate pinned no VS Code version, so it silently tracked the auto-update. Superseded by **v5.4.2** (VS Code version pin). Tag left in history per the forward-only dead-tag precedent. |
| **v5.4.3**   | **Tracker + AGT installer**    | ✅ RELEASED    | Published to VS Code Marketplace + Open VSX (tag `2c7b49c`; GitHub Release auto-created by the new `github-release` pipeline job; META_LEDGER #417). Build & Test cleared a single idle-scheduler timer flake on re-run (B-BIC-24 de-flake follow-up). Premium **Development Tracker** (B-INT-17, PR #141) — data-driven status dashboard at `/console/tracker`, embedded in the Workspace tab (+ Pop out ↗ + `FailSafe: Open Development Tracker`); full-history CHANGELOG discovery (v0.1.0→current), major-default timeline zoom with drill-in, traceable record per release, tiered program-progress (majors full / minors `minorDays` / patches `patchDays`, configurable via `programs.yaml`). Generated organically from `docs/roadmap/programs.yaml`. Plus the **Agent Governance Toolkit installer** (B-INT-16, PR #140) — Integrations → Agent Governance sub-view, env-auto-detected, registry-verified, run-in-terminal. PRs #140 + #141. |
| **v5.4.2**   | **Integration Expansion**      | ✅ RELEASED    | Published to VS Code Marketplace + Open VSX (tag `fdbf779`; GitHub Release created; META_LEDGER #416). Release-gate hotfix that pins the VS Code test version to last-known-good **1.122.1** (`.vscode-test.mjs` + `scripts/rebuild-vscode-electron.cjs`; overridable via `FAILSAFE_VSCODE_TEST_VERSION`), decoupling the release gate from VS Code stable's auto-update so an Electron bump can't dead-tag a release. Carries the full v5.4.0 integration + governance batch and the v5.4.1 stale-test fix. Shipped integrations #99/#100/#108/#95 + research gate #109 closed. PRs #121–#136. **Follow-up filed:** run `test:all` on release-class PRs so native-build / UI-test regressions surface before the tag (both prior dead tags were invisible until the tag fired). |
| **v5.5.0**   | **Integration Suite**          | ✅ RELEASED    | Published to VS Code Marketplace + Open VSX (tag `v5.5.0` `201152a`; both publishes approved at the `production` gate; GitHub Release auto-created; META_LEDGER #418). Integration suite expansion — governance hub for the whole AI toolchain. Governed CLI agent wrappers (Continue #104, Aider #107), agent observe/audit (OpenHands #105, Cline/Roo/Kilo #106), Linear #97 + Jira #98 issue→intent import, GitHub PR checks #96, Sentry #102, Teams #101, Playwright MCP, and a Tier 1 supply-chain CI baseline #90. Consolidated + /qor-substantiate-VETO-remediated (4 external-name defects caught against live docs: Aider `--yes-always`, Sentry env-in-query, OpenHands real-SDK shape, Linear case-insensitive) + idle-scheduler de-flake. 15 per-integration READMEs + docs/structure indices; FEATURE_INDEX FX826–FX836. Squash-merged via PR #153 (release PR #159); closes #90/#96/#97/#98/#101/#102/#104/#105/#106/#107. Enhancement #154 filed. |
| **v5.5.1**   | **Maintenance**                | ✅ RELEASED    | Published to VS Code Marketplace + Open VSX (tag `v5.5.1` `f130701`; both publishes production-gate approved; GitHub Release auto-created; META_LEDGER #419). Maintenance patch (squash-merged via PR #162). CI Actions bumped to their Node.js 24 runtimes, SHA-pinned (Dependabot #155–#158): `checkout` v4→v6.0.3, `setup-node` v4→v6.4.0, `upload-artifact` v4→v7.0.1, `download-artifact` v4→v8.0.1 (v8 fails closed on a digest mismatch). The upload-v7 → download-v8 VSIX hand-off ran clean in the publish jobs (validated the one bump PR CI could not exercise). Development Tracker dashboard surfaces the server `{error}` body instead of a bare "HTTP 500". Issues #161 (Windsurf compatibility) + #163 (tracker progress/freshness) + #154 (PR↔issue linkage governance) filed separately. |
| **v5.5.2**   | **Integration UX**             | ✅ RELEASED    | Completes v5.5.0's integration delivery (UX surfacing that shipped wired but invisible). **Integration Catalog** Integrations-tab sub-view (#167) listing the 9 command/config integrations + Slack with live enabled/configured state (secret-safe boolean-only snapshot; FX837–FX839). **Shared install-progress** component + **Open Design live MCP state** (#166; FX840/FX841). **Bicameral team-mode fix** (#165) — interactive Google Drive OAuth routed through the integrated terminal + per-step install timeout. **Tracker graceful degradation** (FX842) — absent planning manifest no longer 503s. Merged via PRs #168 (#165) + #169 (#167/#166 + tracker) with operator-authorized `--admin` over the intentional ruleset; closes #165/#166/#167. jsdom + Playwright (real ConsoleServer + Chromium) verified. |
| **v5.6.0**   | **Agent Client Protocol**      | ✅ RELEASED    | ACP governance — govern any Agent Client Protocol agent (Devin Desktop/Zed/JetBrains) through FailSafe's enforcement engine. **ACP governance adapter** (#172/#173; FX843–845). **Standalone ACP enforce-proxy** (#172/#178) — reuses the REAL `EnforcementEngine` via vscode-free file-backed backing; enforce blocks out-of-scope fs-writes + denies dangerous permissions; observe/assist record enforcing=false; 7 e2e enforcement proofs + a live MITM `initialize` handshake; CJS `dist/acp-proxy.js`; **governed-twin install UX** (FX848–856 + ajv-instance static-schema fix). **Devin Desktop host detection** (#161/#175; FX846). **Tracker PR-incremental/non-semver resilience** (#174/#176; FX847). 4 PRs `--admin`-merged to main (#173/#175/#176/#178) over the intentional ruleset; full `test:all` green; META_LEDGER #421 (SUBSTANTIATE) + #422 (DELIVER). B-INT-15 (deep content/argv policy) deferred. |
| **v5.6.1**   | **Tracker for any repo**       | ✅ RELEASED    | Development Tracker generator + operator categorization (#174). **Non-semver render resilience** (FX847 Part 2) — PR-incremental repos now populate (was blank-shell). **Manifest generator** (FX857) — `FailSafe: Generate Tracker Manifest` scaffolds `programs.yaml` from merged PRs + CHANGELOG (gh/git/fs, read-only, degrade-safe). **Bicameral-MCP Layer 3 enrichment** (FX858) — decision-aware verticals + decisions ledger when connected. **Operator categorization** (FX859) — programs/verticals are a human taxonomy: interactive keep/drop/rename/fold confirm feeds pure `applyCategoryDecisions`, runs after Bicameral enrichment. NEW Playwright e2e `tracker-dashboard.spec.ts` 3/3 closes the user-facing-visual gate. PRs #183 + #184 (release) `--admin`-merged to main (tag `v5.6.1` `d3d7f57`); published to VS Code Marketplace + Open VSX (both verified live at 5.6.1; production-gate approved; Build & Test cleared one pre-existing FX525 teardown flake on re-run). META_LEDGER #423 (SUBSTANTIATE) + #424 (DELIVER). Closes #174. |
| **v5.6.2**   | **Tracker UX + CI stability**  | ✅ RELEASED    | **Tracker loading/freshness** (#163, FX860) — loading skeleton ("Building the tracker…" + shimmer) replaces the brief blank, a "last refreshed" stamp, and a manual ↻ Refresh; pure client-side in `tracker-dashboard.html` (no server contract change); load/empty/error/success all legible; 3 Playwright e2e + visual verification. **FX525 de-flake** — root-caused the leaked `ConsoleLifecycleService` `ledgerWatcher` that dead-tagged v5.6.1's pipeline; the test helper `close()` now calls `server.stop()` (36/36 under 4×3 stress). PR #187 `--admin`-merged (tag `v5.6.2` `87d3e9a`); **Build & Test passed on the first pipeline run — the de-flake held, no dead-tag**; published (Open VSX live at 5.6.2; VS Code Marketplace job succeeded, query-API ingestion lag at seal time). META_LEDGER #425 (SUBSTANTIATE) + #426 (DELIVER). Closes #163. |
| **v5.6.3**   | **Governance bundle**          | ✅ RELEASED    | Four governance features in one patch. **PR↔issue linkage governance** (#154, FX861 auditor + FX864 live-wiring) — a Check Run flags the `Closes #1, #2` footgun (GitHub closes only the first); pure auditor (9 tests) + `publishLinkageCheck` bridge + command glue (27 github-checks tests). **Tracker-as-governance-sidecar** (FX862) — `governance-projection.ts` projects a `TrackerManifest` from META_LEDGER + FEATURE_INDEX (governed-repo authoritative source; FX857 generator = ungoverned fallback); 6 tests + real-data smoke. **Shadow-genome consumer** (#118, FX863) — pure parse/summarize/governanceSubgraph + off-by-default `loadShadowGenome` over qor-logic's causal graph; 7 tests + real-Python smoke. Three views of the governance ledger (the convergence in the research brief). META_LEDGER #427–#430 (SUBSTANTIATE) + #431 (DELIVER). Release PR #193 `--admin`-merged to main (tag `v5.6.3` `e716767`); published to VS Code Marketplace + Open VSX (both publish jobs succeeded, production-gate operator-approved; Open VSX query-API ingestion lag at seal — publish job authoritative, no reshape). Build & Test passed first-run (FX525 de-flake held). Deferred follow-ups filed: #194 (A.2 sidecar emission), #195 (A.1b plans→phases), #196 (#118 UI), #197 (shared GovernanceProjection substrate), #198 (taxonomy polish). Closes #118, #154. |
| **v5.6.4**   | **Tracker accuracy + visual**  | ✅ RELEASED    | The tracker-as-governance-sidecar thread completed end-to-end. **A.2 sidecar emission** (#194/FX865) + **A.1b plans→programs/phases** (#195/#198/FX866) + **A.2b-data route projection** with unanchored reconciliation (#203/FX867). **Verticals = the 7 Console product surfaces** (not code dirs; `CONSOLE_VERTICALS`, drift-guarded) (#204/FX868) + live `programs.yaml` accuracy + graceful no-server message (#205). **FEATURE_INDEX `Surface` column** on all 689 rows + per-surface `featureStats` attribution (#206/FX869–871); upstream gate **Qor-logic#196** shipped in qor-logic 0.103.1 (surface-tag lint). **A.2b-visual render** — per-vertical coverage bar + program bars that count projected (unanchored) phases + `manifestSource` advisory (#202/FX872–874); Playwright 9/9 + operator visual sign-off. META_LEDGER #432–#439 (SUBSTANTIATE) + #440 (DELIVER). PRs #199–#208 `--admin`-merged to main. Closes #198, #202, #206. |
| **v5.7.0**   | **Shadow Genome + theme inheritance** | ✅ RELEASED    | Published to VS Code Marketplace + Open VSX (tag `v5.7.0` `a6aea0e`; Release Pipeline run 27289811359 — SemVer Gate + Build & Test PASS, both publish jobs succeeded after production-gate approval; GitHub Release created non-draft). Decision-traceability surface + Monitor theme parity. **Shadow Genome governance dashboard** (#196; FX875–FX880) — new **Governance › Shadow Genome** sub-view over `GET /api/qor/governance-dashboard`: deterministic zoomable **Genome Map** of the causal governance graph, **incident ledger** + slide-over case-file drawer, and trust/federation/learning-maturity panels (render real API data, honest-empty until upstream qor-logic producers land); keyboard-operable with a "View as Table" a11y fallback. **Monitor sidebar inherits the Console theme** (#212; FX881) — the embedded Console announces its active theme via `postMessage` and the sidebar chrome mirrors it (tokens inlined from disk, not a CORS-blocked fetch); degrades to hardcoded `var()` fallbacks. FEATURE_INDEX FX876 reconciled `implemented`→`verified`. META_LEDGER #441–#446 + #448 (SUBSTANTIATE) + #447 (RECONCILIATION) + #449 (DELIVER). PRs #211 (#196) + #212 (FX881) + #215 (release) `--admin`-merged to main. Closes #196. |
| **v5.8.0**   | **Shadow Genome lives + governed Organize** | ✅ RELEASED    | The Shadow Genome dashboard stops being empty and the UI cleanup commands stop leaving drift. **#213 producer wiring** (#196 Phase 5; FX883) — trust/federation/learning-maturity now populate from qor-logic 0.111.0's `to_dict` producer surfaces; degrade-safe to honest-empty. **Historical genome reconstruction** (FX884) — a FailSafe-downstream, UNGOVERNED appendix reconstructs the genome map + incidents + maturity from META_LEDGER (GATE/VETO→failure, seals→checkpoint, reuses `parseMetaLedgerEntries`), ingested alongside any real `.qor/genome.jsonl` with **per-record provenance** (recorded vs reconstructed) so a mixed graph stays honest; never writes qor's genome, not Merkle-chained. **Canonical META_LEDGER entry parser** (#197 Slice 1; FX882) — dedup of the `### Entry #N` parse; the view-unifying engine deferred (Hickey). **Organize/Initialize self-commit→push→PR** (FX885) — operator-confirmed governed commit for the Monitor cleanup commands; degrade-safe ladder; decline → audit-log record. qor-logic upgraded 0.109.5→0.111.0. META_LEDGER #450/#452/#454 (RESEARCH) + #451/#453/#455/#456 (SUBSTANTIATE). PRs #217–#221 `--admin`-merged to main. DELIVER seal at close. |
| **v5.9.0**   | **Command Center suite — Tracker / Mind Map / Shadow Genome / Taxonomy** | 🚀 RELEASING   | The seven-issue Command Center remediation from the 2026-06-11 research brief (META_LEDGER #458) + an operator-directed taxonomy editor. **Tracker** — idempotent render (no reload on hub refresh) + full-bleed layout (FX886); programs ∥ verticals taxonomy contract + agent-mapping scaffold (FX887); native print/PDF export (FX888). **Mind Map** — repository knowledge seed projected from the governance/genome graph (auto-seed on empty + REPO / CLEAR LAYER controls), kept distinct from operator brainstorm work (FX889). **Shadow Genome** — "Failure Nodes"→"Graph Nodes" label fix + Observed-count invariant + table-default at scale + label truncation (FX890). **Workspace › Taxonomy editor** (FX891) — edit programs/verticals/agents → governed `docs/roadmap/tracker-config.yaml` + emitted directive + CLAUDE.md force-discover binding; reuses FX887 derive/validate. Four highest-risk plans passed mandatory Option-B independent audit (one VETO→amend→PASS). PR #223 `--admin`-merged to main (full `test:all` green). DELIVER seal at close. |
