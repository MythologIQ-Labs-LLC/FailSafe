...
- [ ] **[B-B199-1] HIGH — Brainstorm behavioral E2E**: workspace-tab.spec.ts confirms Mindmap sub-pill activates but no drawing/voice-input/modal-focus-trap/export/history-replay coverage. Mocha covers `brainstorm-canvas.test.ts` + `silence-timer.test.ts` + `AudioVaultService.test.ts` but not live UI flow.
- [ ] **[B-B199-2] HIGH — Agents Replay + Genome behavioral E2E**: Phase 4 sub-pill click verified; replay timeline scrubbing, snapshot diff rendering, time-travel state restoration, genome failure-pattern card render — zero E2E. Marquee v5 features (B146-B149 Agent Run Replay project).
- [ ] **[B-B199-3] MEDIUM — Voice substrate live behavior**: voice-pack.spec.ts stubs the GitHub Release download via page.route(). Real Whisper / multilingual transcription / Piper audio playback uncovered. Acknowledged trade-off; voice ships separately as companion download.
- [ ] **[B-B199-4] MEDIUM — Cross-host install E2E**: mocha-only via installSkillsHandler.test.ts. Live install flow against claude/codex/kilo-code/gemini hosts uncovered.
- [ ] **[B-B199-5] MEDIUM-HIGH — CI gate hardening**: `check-e2e-coverage.cjs` `[no-e2e: <reason>]` token treated as blanket override across the push range (lines 78-86). Should be scoped per-file. Plus `--no-verify` bypass has no audit trail. Recommend converting gate into release-time check against merge commit range, not developer-bypassable pre-push hook.
- [ ] **[B-B199-6] LOW — Stub-only specs giving false confidence**: `integrations-bicameral.spec.ts` + `voice-pack.spec.ts` use stubs/page.route — would NOT pass against real installs. Track explicitly; either accept the trade-off (separate-download / supply-chain trust posture) or add a CI-gated real-subprocess integration test.
