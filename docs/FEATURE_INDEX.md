...
| FX591 | Educational lesson registry + `getLesson` (Educational Component) | docs/plan-qor-educational-component.md (Phase 1) | src/education/lessons.ts | src/test/education/lessons.test.ts | verified | 8 cases: every `Lesson` has non-empty id/anchor/term/body; anchors unique; declared levels ∈ {beginner,intermediate,advanced}; `getLesson` returns the level-appropriate body + the documented fallback when a level is absent. |
...
| FX598 | Lesson-anchor coherence check (Educational Component) | docs/plan-qor-educational-component.md (Phase 5a) | src/test/education/lesson-anchor-coherence.test.ts | src/test/education/lesson-anchor-coherence.test.ts | verified | 6 cases: passes on the shipped registry; fails (caught) when a lesson anchor points at a removed surface (fixture); classifies webview-mounted vs native-mounted anchors (advisory A3) so native lessons don't false-positive. |
...
| FX601 | FailSafe Glossary section — Playwright (Educational Component, Phase 6) | docs/plan-qor-educational-component.md (Phase 6c) | src/roadmap/ui/modules/education-glossary.js; src/roadmap/ui/modules/settings.js (mount) | src/test/ui/command-center-glossary.spec.ts | verified | 2 Playwright cases: the Settings "FailSafe Glossary" section expands and a term opens to its explanation; the section is absent when education is disabled. |
