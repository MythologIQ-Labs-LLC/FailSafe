...
| FX597 | SHIELD phase-tracker micro-lesson — Playwright (Educational Component) | docs/plan-qor-educational-component.md (Phase 4) | src/roadmap/ui/modules/monitor-render.js | src/test/ui/monitor-shield-lesson.spec.ts | verified | 3 Playwright cases: the Monitor SHIELD phase-tracker "What does this mean?" expander opens to the phase explanation (Plan + Audit); no expander when education disabled. |
| FX598 | Lesson-anchor coherence check — extended with essay-list mount class (FailSafe Learn v2) | docs/plan-qor-failsafe-learn-swe-craft.md (Phase 4); originally Phase 5a in v1 | src/test/education/lesson-anchor-coherence.test.ts | src/test/education/lesson-anchor-coherence.test.ts | verified | 6 baseline cases (v1) + 5 glossary-class cases (FX602 sibling) + essay-list mount class extension (v2): the 5 `learn.essay.*` anchors classify to the essay-list surface (`learn-essay-list.js`); a dead essay anchor would be caught (parallel to the v1 fixture). The v3 track-command extension has been reverted (devCycleTrack removed). |
| FX599 | Glossary lesson class — `kind` discriminator + `glossaryLessons()` (Educational Component, Phase 6) | docs/plan-qor-educational-component.md (Phase 6a/6b) | src/education/lessons.ts (`kind?`, `lessonKind`, `glossaryLessons`); src/education/glossary-content.ts; src/education/glossary-content-2.ts | src/test/education/glossary-lessons.test.ts | verified | 8 cases: `kind` defaults to `'moment'` at read time (4 existing literals omit it — A1); `glossaryLessons()` returns exactly the 12 `'glossary'`-kind entries; every glossary lesson well-formed (id/anchor/term/body, 3 levels). |
| FX600 | `renderGlossary` — FailSafe Glossary section (Educational Component, Phase 6) | docs/plan-qor-educational-component.md (Phase 6c) | src/roadmap/ui/modules/education-glossary.js; src/roadmap/ui/command-center.css | src/test/education/education-glossary.test.ts | verified | 7 jsdom cases: lists all 12 glossary terms as collapsed expanders; empty string when education disabled; proficiency selects the body; term bodies HTML-escaped. |
| FX601 | FailSafe Glossary section — Playwright (FailSafe Learn v2 — secondary reference) | docs/plan-qor-failsafe-learn-swe-craft.md (Phase 5); originally Phase 6c in v1 | src/roadmap/ui/modules/education-glossary.js; src/roadmap/ui/modules/learn.js (mount) | src/test/ui/command-center-glossary.spec.ts | verified | 2 Playwright cases: the Learn-tab "FailSafe Glossary" section expands and a term opens to its explanation; both the essay list (`#cc-learn-essay-list`) and the glossary are absent when education is disabled (single discipline). Glossary is the secondary reference below the primary essay list. |
| FX602 | Coherence check — `'glossary'`-kind class validation (Educational Component, Phase 6) | docs/plan-qor-educational-component.md (Phase 6d) | src/test/education/lesson-anchor-coherence.test.ts | src/test/education/lesson-anchor-coherence.test.ts | verified | 5 cases: `'glossary'`-kind lessons positively validated against the glossary surface (advisory A2 — a dead glossary lesson is caught, not exempted); no false-positives against governance-moment anchors; FX598's 6 governance-moment cases still pass. |
| FX606 | `LearnRenderer` — Learn tab host (FailSafe Learn v2) | docs/plan-qor-failsafe-learn-swe-craft.md (Phase 3) | src/roadmap/ui/modules/learn.js; src/roadmap/ui/command-center.html; src/roadmap/ui/command-center.js | src/test/education/learn-tab.test.ts | verified | 3 jsdom cases: composes the SWE-craft essay list (primary) + the FailSafe glossary (secondary); both are gated off when education is disabled (single discipline); the essay list renders 5 cards when enabled. |
