...
Single canonical cross-reference of every user-touchable feature in FailSafe v5.1.0 against documentation, source code, and test surface. Refresh in every `/qor-implement` and `/qor-substantiate` cycle per `feedback_feature_index_every_cycle.md`.
...
| FX487 | GET /api/integrations/bicameral/status | INTEGRATIONS.md (route surface) | src/roadmap/routes/BicameralRoute.ts | — | unverified | Route handler shape ships in this cycle; functional HTTP-harness coverage lands with FX490 Phase 5 fake-spawn Playwright. |
| FX488 | POST /api/actions/bicameral-install bridge | INTEGRATIONS.md (route surface) | src/roadmap/routes/BicameralRoute.ts | — | unverified | Spawn boundary covered by FX485; route argv/mode validation + WS broadcast flow Playwright-covered in FX490. |
| FX489 | POST /api/actions/bicameral-{connect,disconnect,history,drift,ratify} | INTEGRATIONS.md (route surface) | src/roadmap/routes/BicameralRoute.ts | — | unverified | Client surface covered by FX483; route 503/409 short-circuits + happy-path forwarding Playwright-covered in FX490. |
| FX490 | Settings card (Bicameral MCP — status + autoConnect + re-install link) | INTEGRATIONS.md (Settings) | src/roadmap/ui/modules/bicameral-settings-card.js, src/extension/bootstrapBicameral.ts, package.json contributes.configuration | — | unverified | End-to-end Playwright spec lands in Phase 5: stub bicameral-mcp spawn fixture → install picker → install progress → connect → feature feed flow (per session-state Phase 5 task). |
