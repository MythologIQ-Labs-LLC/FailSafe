...
**Current Release:** v6.0.5 (2026-09-04) — **12 merged PRs**: six that had been waiting (#438 FX932 seal, #442 Bicameral upstream error state, #443 plan-grep-lint regex recognition, #444 per-engine verification provenance, #445 Shadow Genome severity aria, plus two dependency bumps) and the four governance-integrity cycles (#448-#451, ledger #602-#605). Five controls were found reporting success while inspecting nothing; each now carries a falsifier. Four issues filed upstream (Qor-logic #425-#428). **v6.0.4 still has no `DELIVER` ledger entry** — unchanged by this release, owned by `/qor-repo-release`.
...
**Release scope correction.** An earlier attempt at this release tagged and pushed v6.0.5 while six PRs were still open — four of them mergeable with zero failures, and two conflicting only because three consecutive governance cycles had edited `FEATURE_INDEX.md` and `GOVERNANCE_INDEX.md` out from under them. The operator declined to approve the production gate, the pipeline was cancelled, and the tag was deleted before anything published. All six were then merged — the two conflicts rebased and re-verified — and the release re-cut at a head that contains the work. Recorded because the failure was not the conflict: it was proceeding to tag after having already identified and reported the problem.
...
3. **FailSafe Pro Discovery**: Command palette `FailSafe: About FailSafe Pro` and Settings panel card link to canonical URL `https://mythologiq.studio/products/failsafe-download`. Single-source-of-truth constant in `src/shared/constants.ts` with drift-guard test.
