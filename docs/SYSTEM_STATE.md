...
**Current Release:** v5.6.2 (DELIVER seal #426; Open VSX live + VS Code Marketplace publishing — tracker loading/freshness + FX525 de-flake; Build & Test passed first-run). Working state: **#154 PR↔issue linkage auditor** (FX861, SUBSTANTIATE #427) **MERGED to main** (`5aa4cee`) — pure `pr-linkage-audit.ts` governing the `Closes #1, #2` footgun, beside the #96 SHIELD check; awaits a release. Plus **Tracker GovernanceProjection reader** (FX862, A.1 — the tracker-as-governance-sidecar first slice): pure `governance-projection.ts` projects a `TrackerManifest` from META_LEDGER (releases + decisions) + FEATURE_INDEX (verticals) — the governed-repo authoritative source (FX857 generator = ungoverned fallback). On branch `feat/governance-projection-tracker` (SUBSTANTIATE seal #428 — held local, awaits `--admin` merge + release decision). 6 unit tests + real-data smoke (12 releases / 15 verticals / 54 decisions), tsc 0, lint clean. Research brief: `.failsafe/governance/research-brief-tracker-governance-sidecar-2026-06-06.md`.
