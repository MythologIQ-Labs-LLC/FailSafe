...
**Current Release:** v5.6.2 (DELIVER seal #426; Open VSX live + VS Code Marketplace publishing — tracker loading/freshness + FX525 de-flake; Build & Test passed first-run). Working state: **GitHub PR↔issue linkage governance auditor** (#154, FX861) — pure `pr-linkage-audit.ts` (parse close-keywords per-number → findings → Check Run conclusion) + off-by-default injected-transport live path, beside the #96 SHIELD-verdict check; governs the `Closes #1, #2` footgun. On branch `feat/pr-linkage-governance-154` (SUBSTANTIATE seal #427 — held local, awaits operator `--admin` merge + release decision). 9 unit tests, tsc 0, lint clean.
