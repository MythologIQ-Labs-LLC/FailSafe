...
**Current Release:** v5.6.2 (DELIVER seal #426; Open VSX live + VS Code Marketplace publishing — tracker loading/freshness + FX525 de-flake; Build & Test passed first-run). Working state: a **3-feature governance bundle queued for v5.6.3**, all unreleased: (1) **#154 PR↔issue linkage auditor** (FX861, #427) MERGED to main (`5aa4cee`); (2) **Tracker GovernanceProjection reader** (FX862, A.1, #428) MERGED to main (`0e54563`) — the tracker-as-governance-sidecar first slice (pure `governance-projection.ts` projecting a `TrackerManifest` from META_LEDGER + FEATURE_INDEX, the governed-repo authoritative source); (3) **Shadow-genome client** (FX863, #118, #429) — consumes qor-logic's `shadow_genome_graph` causal graph (pure parse/summarize/governanceSubgraph + off-by-default injected-RunCommand loader; 7 tests + real-Python smoke) on branch `feat/shadow-genome-viewer-118` (SUBSTANTIATE seal #429 — held local, awaits `--admin` merge). #154 + A.1 on main, #118 in PR; all await the **v5.6.3 bundle** release. Research brief: `.failsafe/governance/research-brief-tracker-governance-sidecar-2026-06-06.md`.
