...
| FX492 | NEW | src/test/extension/voice-pack-install.test.ts | `installVoicePack(context, opts)` downloads via stubbed Node-20 `fetch({ redirect: 'follow' })`, verifies SHA256, extracts to staging, atomic-renames to final; cleanup on failure leaves prior pack intact; rejects non-GitHub redirect targets via bounded-allowlist |
| FX493 | NEW | src/test/roadmap/voice-pack-settings-card.test.ts | Settings "Voice Pack" card renders `absent / installed v<X> / stale / error` states with correct buttons (Install / Update / Uninstall / Dismiss + Retry on error) + version display + disk-usage line; error state surfaces last failed `InstallProgressEvent.error` text |
...
- `FailSafe/extension/src/test/extension/voice-pack-install.test.ts` — NEW. 7 cases: `installVoicePack` invokes `fetch` with correct URL (resolved from extension version) AND `{ redirect: 'follow' }` option (verifies F3 remediation — Node 20 built-in fetch following GitHub Releases 302); SHA256 mismatch aborts before extract; extract failure leaves staging dir without renaming over prior pack; success path atomic-renames; uninstall removes globalStoragePath/voice-pack/ directory; rejects malformed pack URL via spawn-boundary allowlist (mirrors `isSafeBicameralCommand`); rejects non-GitHub redirect targets (stub the fetcher to return a 302 to `evil.example.com/...`; assert installVoicePack rejects without piping any body to disk — bounded-redirect allowlist).
...
- `FailSafe/extension/src/voice-pack/install-handler.ts` — NEW. `installVoicePack(opts: { globalStoragePath, version, output, onProgress }): Promise<InstallReport>`. Resolves pack URL via `resolveVoicePackUrl(version) -> ` `https://github.com/MythologIQ/FailSafe/releases/download/v<X.Y.Z>-voice/failsafe-voice-pack-<X.Y.Z>.tar.gz`. Downloads via Node 20+ built-in `fetch(url, { redirect: 'follow' })` — VS Code 1.90+ engine (declared in `package.json.engines.vscode`) guarantees Node 20.x runtime, where `fetch` is stable and follows HTTP 302 redirects by default. This is required for GitHub Releases asset URLs which respond with 302 → `objects.githubusercontent.com/...`; the prior plan iteration cited `node:https.get` which does NOT follow redirects (F3 remediation, audit cycle 1). Pipe response body to `<staging>/voice-pack.tar.gz` via `Readable.fromWeb(response.body).pipe(createWriteStream(...))` (stdlib only — `node:stream`, `node:fs`). Concurrently `fetch` the `.sha256` companion (same redirect-follow); verify; extract via `child_process.spawn('tar', ['-xzf', staging, '-C', staging], { shell: false })`; reads extracted manifest; atomic `fs.renameSync(staging, finalPath)`. Failure cleans staging. `uninstallVoicePack(globalStoragePath)` rm-rf's the voice-pack/ directory only. **Note on `tar` cross-platform availability** (audit cycle 1 observation O1): VS Code 1.90+ engine requirement guarantees Windows 10 1809+ which ships `tar.exe`, macOS ≥ 11 ships BSD tar, Linux all-distributions ship GNU tar — `tar` is universally available across the supported platform matrix.
- `FailSafe/extension/src/voice-pack/index.ts` — NEW. Barrel.

### Changes

The substrate is read-mostly: `probeVoicePackState` is a pure function over filesystem state; `installVoicePack` is a sequence of `fetch(url, { redirect: 'follow' })` + `crypto.createHash` + `Readable.fromWeb` stream-to-file + `spawn(tar, [...])` calls under explicit operator action. No new npm dependencies (Node 20+ stdlib only: built-in `fetch`, `node:stream` for `Readable.fromWeb`, `node:crypto`, `node:fs`, `node:child_process`). Pack URL allowlist mirrors `isSafeBicameralCommand` shape: only `https://github.com/MythologIQ/FailSafe/releases/download/v<semver>-voice/failsafe-voice-pack-<semver>.tar.gz` patterns pass; the `redirect: 'follow'` chain is bounded to GitHub-hosted destinations (`github.com` and `objects.githubusercontent.com` only) — any non-GitHub redirect target rejects the download.
...
- `FailSafe/extension/src/test/roadmap/voice-pack-settings-card.test.ts` — NEW. 5 JSDOM cases for FX493 (brainstorm-voice-button scope removed per audit cycle 1 F2 — covered by FX495 Playwright): card renders `absent` state with Install button + version-empty line + "Voice features disabled" hint; renders `installed v<X>` state with version + Uninstall + Update (when stale) + disk-usage line read from `du`-equivalent stat; renders `error` state with last failed `InstallProgressEvent.error` message + Dismiss button (returns card to prior state via state-rollback) + Retry button (re-POSTs `/api/actions/install-voice-pack`); toggle click POSTs `/api/integrations/voice-pack/{install,uninstall}`; slot is removed when `/api/integrations/voice-pack/status` returns 404 (build without voice substrate).
...
- `FailSafe/extension/src/roadmap/ui/modules/voice-pack-settings-card.js` — NEW. ≤ 100 lines (raised from ≤80 to accommodate error-state render per F1 remediation). Card render + bind, parallel to `bicameral-settings-card.js`. Reads `GET /api/integrations/voice-pack/status`, renders state-appropriate buttons + version + disk-usage line. **Four render states**: `absent` (Install button), `installed v<X>` (Uninstall + version + disk-usage), `stale` (Update + Uninstall + version), `error` (last `InstallProgressEvent.error` text + **Dismiss** + **Retry** buttons; Dismiss returns the card to its pre-install state, Retry re-POSTs `/api/actions/install-voice-pack`). The card consumes `voicePack.install.progress` WS events to drive per-phase intermediate states (download / verify / extract / manifest-verify) — satisfies Live-Progress Invariant SG-FakeProgress-A by surfacing each phase + bytesTransferred for the download phase, plus an explicit operator-dismiss path on terminal error.
